...
### 功能特点
- **动态人设**: 支持在线创建、切换AI角色，无需重启服务
- **人设持久化**: 角色数据自动保存，重启后仍然可用
- **角色隔离**: 切换角色后会自动清除对话历史，确保角色体验纯净
...
3. **添加角色**: `.chat chara add <角色名> <描述>` - 创建新角色
...
# 添加一个法师角色
.chat chara add wizard 你是一位博学的法师，精通各种魔法知识，善于用魔法的角度解释问题，说话带有古典魔法师的风格

# 添加一个侦探角色
.chat chara add detective 你是一名经验丰富的侦探，思维敏锐，善于观察细节和逻辑推理，总是能从蛛丝马迹中发现真相

# 切换到法师角色
.chat chara set wizard

# 与法师角色对话
.chat 请教我一些关于魔法的知识

# 切换到侦探角色
.chat chara set detective
...
- 角色描述长度限制为1000个字符
- 切换角色后对话历史会被清除
- 角色数据保存在`backend/characters.json`文件中
